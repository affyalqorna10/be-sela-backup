...
public interface SpesificationRepository extends PagingAndSortingRepository<MstSpesifikasi, Long> {
    @Query(value = "SELECT * FROM mst_spesifikasi AS spek LEFT JOIN mst_device AS device ON spek.device_id = device.device_id order by spek_id asc limit 1",nativeQuery = true)
