...
public interface Stockrepository extends JpaRepository<Stock, Long> {
    ...
    @Query(value = "select ms.* from stocks ms where ms.brand_id= :brandId",nativeQuery = true)
    List<Stock> findByBrandIdAll(@Param("brandId") Long brandId);

    @Query(value = "select ms.* from stocks ms where ms.brand_id= :brandId",nativeQuery = true)
    List<Fiturdetaildto> findByBrandIdlList(@Param("brandId") Long brandId);

    @Query(value = "select ms.* from stocks ms where ms.id= :Id",nativeQuery = true)
